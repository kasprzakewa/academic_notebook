...
function test_max_flow_sap(i::Int64, printFlow::Bool)
    vertices = generate_vertices(i)
    edge_count = generate_edges(vertices, i)
...
function test_for_plots()
    for i in 1:16
        ...
        for j in 1:10
            ...
            max_flow_avg += max_flow
            time_ek_avg += time
            time_sap_avg += time1
            found_paths_avg += found_paths

            open("test/ek.txt", "a") do file
                println(file, i, " ", time)
            end

            open("test/sap.txt", "a") do file
                println(file, i, " ", time1)
            end

            open("test/utils.txt", "a") do file
                println(file, i, " ", max_flow, " ", found_paths)
            end
        end
        ...
        open("test/sap_avg.txt", "a") do file
...
function main()
    ...
    test_both(args["size"], args["printFlow"], nothing)
end
