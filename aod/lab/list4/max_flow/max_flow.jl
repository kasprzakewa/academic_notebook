...
mutable struct Vertex
    ...
    Vertex(value, label, h, labeled, num_of_edges, neighbours, last_visited, d) = new(value, label, h, labeled, num_of_edges, neighbours, last_visited, d)
    Vertex(value, label, h) = new(value, label, h, false, 0, Vector{Tuple{Vertex, Int64, Int64}}(), 0, typemax(Int64))
end
    ...
    for i in 1:length(vertices)
        ...
        for j in (i+1):length(vertices)
            ...
            if sum(v.label .!= w.label) == 1
                ...
                push!(v.neighbours, (w, capacity, 0))
...
function shortest_augmenting_path(vertices::Vector{Vertex}, start::Vertex, finish::Vertex)::Tuple{Int64, Int64}
    max_flow = 0
    found_paths = 0
    distance_count = distance_label(vertices, finish)
    max_distance = length(vertices)
    pred = Dict{Vertex, Vertex}()
    i = start
    ...
    while start.d < length(vertices)
        ...
        if j != nothing
            ...
            old_distance = i.d
            retreat(i)
            new_distance = min(i.d, max_distance)

            distance_count[old_distance + 1] -= 1
            distance_count[new_distance + 1] += 1

            if distance_count[old_distance + 1] == 0 && old_distance < length(vertices)
                break
            end
...
function distance_label(vertices::Vector{Vertex}, sink::Vertex)
    max_distance = length(vertices)
    distance_count = zeros(Int64, max_distance + 1)
...
function has_admissible_arc(v::Vertex)::Union{Vertex, Nothing}
    for i in (v.last_visited + 1):length(v.neighbours)
...
function save_to_model(vertices::Vector{Vertex}, k::Int64, printFlow::Bool, filename::String)
    ...
    open("$filename", "w") do file 
        ...
        println(file, "time_all = @elapsed begin")
        ...
        println(file, "")
        println(file, "open(\"./test/mf_lp_table.log\", \"a\") do stderror")
        println(file, "    redirect_stderr(stderror) do")
        println(file, "        println(stderror, k, \" \", objective_value(model))")
        println(file, "    end")
        println(file, "end")
        println(file, "")
        println(file, "open(\"./test/mf_lp.log\", \"a\") do stderror")
        println(file, "    redirect_stderr(stderror) do")
        println(file, "        println(stderr, k, \" \", time_max_flow, \" \", time_all)")
        println(file, "    end")
        println(file, "end")
