...
k = 3
degree = 1
vertices = [i for i in 0:(2^(k+1) + 1)]
edges = read_edges("edges.txt")

start = vertices[1]
finish = vertices[2^(k+1) + 2]
...
time_bipartite = @elapsed begin
 optimize!(model)
end
end

println("MATCHING SIZE: ", objective_value(model))

open("./test/mf_lp_table.log", "a") do stderror
    redirect_stderr(stderror) do
        println(stderror, k, " ", degree, " ", objective_value(model))
    end
end

open("./test/bm_lp.log", "a") do stderror
    redirect_stderr(stderror) do
        println(stderr, k, " ", degree, " ", time_bipartite)
    end
end
