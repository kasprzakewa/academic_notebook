...
function save_to_model(filename::String, graph::Graph_struct, s_neighbours::Vector{Tuple{Vertex, Int64, Bool}}, k::Int64, degree::Int64, printFlow::Bool)
    ...
    open("$filename", "w") do file
        ...
        println(file, "        println(stderror, k, \" \", degree, \" \", objective_value(model))")
        println(file, "    end")
        println(file, "end")
        println(file, "")
        println(file, "open(\"./test/bm_lp.log\", \"a\") do stderror")
        println(file, "    redirect_stderr(stderror) do")
        println(file, "        println(stderr, k, \" \", degree, \" \", time_bipartite, \" \", time_all)")
        println(file, "    end")
        println(file, "end")
