...
real = -0.0042963432  # Rzeczywista wartość sumy iloczynów
...
# Wyświetlenie danych w formacie latex
println("\\begin{table}[h!]")
println("\\centering")
println("\\begin{tabular}{|c|c|c|c|c|}")
println("\\hline")
println("\\textbf{Typ} & \\textbf{W przód} & \\textbf{W tył} & \\textbf{Max -> Min} & \\textbf{Min -> Max}\\\\")
println("\\hline")

println("Float32 & $(scalar_forward(a, b, Float32)) & $(scalar_backwards(a, b, Float32)) & $(scalar_max_min(a, b, Float32)) & $(scalar_min_max(a, b, Float32))")
println("\\hline")
println("Float64 & $(scalar_forward(a, b, Float64)) & $(scalar_backwards(a, b, Float64)) & $(scalar_max_min(a, b, Float64)) & $(scalar_min_max(a, b, Float64))")
println("\\hline")
println("Float64 (real) & $real & $real & $real & $real")
println("\\hline")

println("\\end{tabular}")
println("\\caption{Porównanie sumowania iloczynów elementów wektorów w różnych kolejnościach.}")
println("\\label{table:scalar}")
println("\\end{table}")